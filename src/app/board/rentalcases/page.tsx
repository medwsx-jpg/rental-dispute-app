...
export default function RentalCasesPage() {
  ...
  const loadPosts = () => {
    const q = query(collection(db, 'boards', 'rentalcases', 'posts'), orderBy('timestamp', 'desc'));
  ...
  const handlePostClick = async (post: Post) => {
    try {
      await updateDoc(doc(db, 'boards', 'rentalcases', 'posts', post.id), { views: increment(1) });
    } catch (error) {
      console.error(error);
    }
  ...
  const handleAddComment = async (postId: string) => {
    ...
    try {
      await updateDoc(doc(db, 'boards', 'rentalcases', 'posts', postId), {
