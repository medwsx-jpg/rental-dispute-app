...
export default function HouseCasesBoardPage() {
  ...
  const loadPosts = () => {
    const q = query(
      collection(db, 'boards', 'house-cases', 'posts'),
  ...
  const handleDeletePost = async (postId: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'boards', 'house-cases', 'posts', postId));
      alert('게시글이 삭제되었습니다!');
  ...
  const handlePostClick = async (post: Post) => {
    try {
      const postRef = doc(db, 'boards', 'house-cases', 'posts', post.id);
  ...
  const handleAddComment = async (postId: string) => {
    ...
    setCommenting(true);
    try {
      const postRef = doc(db, 'boards', 'house-cases', 'posts', postId);
      await updateDoc(postRef, {
        comments: arrayUnion({
          userId: user.uid,
          userNickname: nickname,
      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="text-center">
            <h1 className="text-xl font-bold text-gray-900">🏠 부동산 분쟁사례</h1>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          ...
          <p className="text-sm text-blue-700">
            입주 전 사진이 없어서 보증금을 돌려받지 못한 실제 사례들입니다. Record365로 미리 예방하세요!
          </p>
        </div>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <input
                  ...
                  placeholder="예: 전세 퇴거 시 벽지 교체비 200만원 청구됨"
