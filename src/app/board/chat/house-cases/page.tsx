...
export default function HouseCasesPage() {
  ...
  const loadPosts = () => {
    const q = query(collection(db, 'boards', 'house-cases', 'posts'), orderBy('timestamp', 'desc'));
  ...
  const handlePostClick = async (post: Post) => {
    try {
      await updateDoc(doc(db, 'boards', 'house-cases', 'posts', post.id), { views: increment(1) });
    } catch (error) {
      console.error(error);
    }
  ...
  const handleAddComment = async (postId: string) => {
    ...
    try {
      await updateDoc(doc(db, 'boards', 'house-cases', 'posts', postId), {
