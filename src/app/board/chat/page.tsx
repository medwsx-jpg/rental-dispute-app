...
export default function RentalCasesPage() {
  ...
  const loadPosts = () => {
    const q = query(collection(db, 'boards', 'rental-cases', 'posts'), orderBy('timestamp', 'desc'));
  ...
  const handlePostClick = async (post: Post) => {
    try {
      await updateDoc(doc(db, 'boards', 'rental-cases', 'posts', post.id), { views: increment(1) });
    } catch (error) {
      console.error(error);
    }
  ...
  const handleAddComment = async (postId: string) => {
    ...
    try {
      await updateDoc(doc(db, 'boards', 'rental-cases', 'posts', postId), {
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="text-center">
            <h1 className="text-xl font-bold">🚗 렌탈 분쟁사례</h1>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-6">
          ...
          <p className="text-sm text-orange-700">사진 기록이 없어서 억울한 비용을 물게 된 실제 사례들입니다.</p>
        </div>
